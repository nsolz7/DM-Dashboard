...
export async function POST(request: Request) {
  ...
  let body: EquipRequestBody;

  try {
    body = (await request.json()) as EquipRequestBody;
  } catch {
  ...
  try {
    ...
    const result = await equipInventoryItem(db, {
    ...
    await safeCreateCampaignTransaction({
      ...
      message: {
        ...
        body: `DM equipped ${result.itemName} for ${result.playerName ?? result.playerId}. ${changeSummary}`,
        severity: "success",
        icon: "equip"
      },
      sender: {
